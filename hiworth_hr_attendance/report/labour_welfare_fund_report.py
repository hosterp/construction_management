...
class LabourWelfareFundWizardReport(models.TransientModel):
	_name = 'labour.welfare.fund.wizard'
	...
	def action_employee_pf_esi_open_window(self):
		...
		return {
			...
			'report_name': 'hiworth_hr_attendance.report_employee_labour_welfare_template',
			'datas': datas,
	...
	def get_esi_pf_details(self):
		...
		employer_eps = 0
		domain = []
		# domain.append(('pt_check','=',True))
		if self.company_contractor_id:
			domain.append(('company_contractor_id','=',self.company_contractor_id.id))
		pf = self.env['labour.welfare.fund'].search([('month', '=', self.month), ('year', '=', self.year)], limit=1)
		employees = self.env['hr.employee'].search(domain)
		for empl_id in pf:

			for line2 in empl_id.labour_welfare_fund_line_ids:

				wages_due = line2.wages_due

				employer_epf = line2.amount
				employee_epf = line2.employer_contribution
				...
				'employee_amt': employer_epf,
				'employer_amt': employee_epf,
