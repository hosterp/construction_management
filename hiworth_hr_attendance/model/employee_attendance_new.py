...
class ContractEntryWizardLine(models.TransientModel):
	_name = 'contract.entry.wizard.line'

	leave_id = fields.Many2one('hr.holidays.status',"Leave Type")
	nos = fields.Float("No of Days")
	contract_entry_wizard_id = fields.Many2one('contract.entry.wizard',"Contract entry wizard")

class ContractEntryWizard(models.TransientModel):
	_name = 'contract.entry.wizard'

	@api.model
	def default_get(self, default_fields):
		vals = super(ContractEntryWizard, self).default_get(default_fields)
		line_ids2 = []
		for line in self.env.context.get('active_ids'):

			line_ids2.append(line)
		vals.update({'contract_ids':[(6,0,line_ids2)]})
		return vals

	contract_ids = fields.Many2many('hr.contract','contract_wizard_contract_rel','contract_entry_id','contract_id',string="Contracts")
	date_from = fields.Date("Date From")
	date_to = fields.Date("Date To")
	contract_entry_wizard_line_ids = fields.One2many('contract.entry.wizard.line','contract_entry_wizard_id',"Leave Details")

	@api.multi
	def do_mass_update(self):
		for rec in self:
			for contra in rec.contract_ids:
				if contra.state == 'active':
					contra.state = 'deactive'
					leaves = []
					for leave in rec.contract_entry_wizard_line_ids:
						leaves.append((0,0,{'leave_id':leave.leave_id.id,
											'nos':leave.nos}))
					new_contract = {'name':contra.employee_id.name + "CONTRACT",
						'employee_id': contra.employee_id.id,
									'type_id': contra.type_id.id,
									'job_id': contra.job_id.id,
									'wage': contra.wage,
									'date_start': rec.date_from,
									'date_end': rec.date_to,
									'struct_id': contra.struct_id.id,
									'schedule_pay': contra.schedule_pay,
									'rule_lines': [(6, 0, contra.rule_lines.ids)],
									'notes': contra.notes,
									'contract_id': contra.id,
									'employee_leave_ids':leaves}
					self.env['hr.contract'].create(new_contract)
				elif contra.state == 'approve':
					contra.state = 'active'
				else:
					return True
	...
	def do_mass_update(self):
		for rec in self:
			...
			for lines in rec.line_ids:
				...
				attendance_date = datetime.strptime(rec.date, "%Y-%m-%d")
